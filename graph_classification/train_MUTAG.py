...
for label in range(2):
  ...
  parameter.labeled_graphs = labeled_graphs[label] & train_graphs
  parameter.graphs = graphs
  parameter.left_graphs = labeled_graphs[label] & train_graphs 
  parameter.train_graphs = train_graphs
  parameter.expected = 1.2

  learned_parameters = learn_abs_graphs_bottom_up(parameter, myMaps)
  with open('MUTAG/learned_parameters_for_{}.pickle'.format(label), 'wb') as f:
